...
public class AppDbContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder builder)
    {
        ...
        builder.Entity<Reserva>().Property(p => p.Id).ValueGeneratedOnAdd();
        ...
        builder.Entity<Reserva>().HasOne(p => p.ServiceProvider).WithMany(p => p.Reservas).HasForeignKey(p => p.ServiceProviderId);
        builder.Entity<Reserva>().HasOne(p => p.ClientPayment).WithMany(p => p.Reservas).HasForeignKey(p => p.ClientPaymentId);
        builder.Entity<Reserva>().HasOne(p => p.Estado).WithMany(p => p.Reservas).HasForeignKey(p => p.EstadoId);
        ...
        builder.Entity<Review>().HasOne(p=>p.User).WithMany(p=>p.Review).HasForeignKey(p => p.UserId);
        builder.Entity<Review>().HasOne(p=>p.Service).WithMany(p=>p.Review).HasForeignKey(p => p.ServiceId);
